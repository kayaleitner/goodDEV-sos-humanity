...
export default function (blockListingRegions) {
  // find the project which is the highest and set the min height of the map to that
  const projectList = blockListingRegions.getElementsByClassName('region-list')[0]
  const projects = blockListingRegions.getElementsByClassName('region')
  ...
  blockListingRegions.querySelector('#dottedMap').innerHTML = svgMap
  ...
  const setRegionsMap = (countries) => {
    ...
    blockListingRegions.querySelector('#regionsMap').innerHTML = regionsMap
  }
